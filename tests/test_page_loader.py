...
def test_page_loader():
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        with requests_mock.Mocker() as mock:
            mock.get(MAIN_URL + PATH, text=get_content("tests/fixtures/mock/HTML.html"))
            mock.get(
            ...
            mock.get(MAIN_URL + "/no-resource.png", status_code=HTTPStatus.NOT_FOUND)
            download(MAIN_URL + PATH, tempdir)
            received_html = get_content(os.path.join(tempdir, HTML_FILE))

            assert received_html == correct_html
            assert os.path.isfile(
                os.path.join(
                    tempdir, FILES_DIR, "ru-hexlet-io-assets-professions-nodejs.png"
                )
            )
            assert os.path.isfile(
                os.path.join(tempdir, FILES_DIR, "ru-hexlet-io-packs-js-runtime.js")
            )
            assert os.path.isfile(
                os.path.join(tempdir, FILES_DIR, "ru-hexlet-io-assets-application.css")
            )
            assert os.path.isfile(
                os.path.join(tempdir, FILES_DIR, "ru-hexlet-io-courses.html")
            )
